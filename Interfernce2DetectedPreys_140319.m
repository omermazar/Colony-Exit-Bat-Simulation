function [Masking2DetectionStruct] = Interfernce2DetectedPreys( PulsePower,...
                        ...
                        InterferenceFullVec,  InterferenceFullStruct, PulseNum, BAT, BatNumRx, AllParams )
...
% detected prey items includeind the loclization errors
...
%             maxTimeCoverInterfernce  = max(AllInterPowerVec(nEchoTimes));
%             [IsCoverTimeJamFlag(k) , CoverTimeJamTimes(k)] = CheckMaskingForMaxPowerTH ...
%                 (CurrentPick, maxTimeCoverInterfernce, MinSignal2InterfererTH, nEchoTimes(1));
%             % Fwd Masking Interfernce
%             maxFwdMaskingInterfernce = max(AllInterPowerVec( (nEchoTimes(1)- FwdMaskingTime) : (nEchoTimes(1)-1) ));
%             
%             [IsFwdMaskingJamFlag(k) , FwdMaskingJamTimes(k)] = CheckMaskingForMaxPowerTH...
%                 (CurrentPick, maxFwdMaskingInterfernce, MaskingFwdSIRth, nEchoTimes(1));
%             
%             IsPreyMaskedFlag(k) = IsCoverTimeJamFlag(k) || IsFwdMaskingJamFlag(k);
%             
%             DetecetedPrey2InterferenceRatioDB(k) =  CurrentPick(k) - max(maxFwdMaskingInterfernce, maxTimeCoverInterfernce);
       %% 
        case 'MaxPowerTH'   %  switch ReceiverType

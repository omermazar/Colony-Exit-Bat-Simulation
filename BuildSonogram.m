function [SonogramMatrix, SonarEchosMat, PreyEchosVec, ObsEchosVec, InterferenceSonogram ] = ...
    BuildSonogram( nTotPulses, TransmittedPulsesStruct ...
...
%     try
     ...
     if calc_obs_echoes_flg
        [SonogramMatrix, SonarEchosMat, ObsEchosVec] = EchosSonogramAssignment(nTotPulses, NumOfTimesObsFinds, EchosFromObsStruct, PulseWidthVec, ...
...
%     end
    ...
    %%% Echos from Prey
%     try
    [SonogramMatrix, SonarEchosMat, PreyEchosVec] = EchosSonogramAssignment(nTotPulses, NumOfTimesPreyFinds, EchosFromPreyStruct, PulseWidthVec, ...
...
function [SonogramMat, SonarEchosMat, EchosVec] = EchosSonogramAssignment(nTotPulses, NumOfTimesFinds, EchosStruct, PulseWidthVec, ...
...
% EchosStruct = EchosFromObsStruct/ EchosFromPreyStruct
    ...
    if NumOfTimesFinds > 0 && any([EchosStruct.NumOfEchos]) % if there ara no finds - do nothing ...
        ...
        CurrIndex = 0;
        for nPulseNumber = 1: nTotPulses % NumOfTimesFinds
            if EchosStruct(nPulseNumber).NumOfEchos > 0
            ...
            end % if EchosStruct(nPulseNumber).NumOfEchos >0

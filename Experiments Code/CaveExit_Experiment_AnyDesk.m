function [Table_FullExperiment ] = CaveExit_Experiment_AnyDesk()
...
%     'D:\Dropbox\University\מחקר\experiments\caveExit\Code\DefaultParamsTable_caveExit.xlsx';


runPath =  'D:\OMER\Bat Simulation'; %'D:\Omer\BatSimulation\All Directories_backup\Bat Simulation';
%%%%%% Select which params to run - Important %%%%%%%%%%%%%%%%%%
% NominalParams_FilePathName = 'D:\OMER\caveExit\Code\DefaultParamsTable_CaveExit.xlsx';
NominalParams_FilePathName = 'D:\OMER\caveExit\Code\DefaultParamsTable_CaveExit_Rhinopoma.xlsx';
%%%%%%                                  %%%%%%%%%%%%%%%%%%%%
...
TotalBatsNumber =  [1 2 5 10 20 40 100]; % [1 2 5 10  20 ]; %; % 20; % 100; %[1 2 5 10 15 20 25 40]; %[ 18 20 22]; %[20 20]; %[1 2 5 10 15 20 25 40];
NumOfSetups1    = numel(TotalBatsNumber); % ([Preg, Post])NumOfSetups1 = 2; % ([Preg, Post])
% % num of repetitions in each setup
nTrials         =  [120 60 24 12 6 4 4];
...
AcousticsMaskFlag = [true, false] ;  % false; %  ; % [true, false]  ; %[true, false]; % [ 0 1 1];
NumOfSetups2      = length(AcousticsMaskFlag);

% % Var3
ClutterGainDB = [-10, 10, 20]; %[0:4];
NumOfSetups3 = length(ClutterGainDB);

%%% parameter for Saving DATA
SavePath   = 'D:\OMER\caveExit\Results\CurrRun'; %/home/omermazar/caveExit_Results'; %'D:\Dropbox\University\מחקר\experiments\caveExit\Results\';
SaveExpDir = '/'; % 'Sep2022\Run\17Oct\';
saveBatDATAflg = 0;

% The experiment
ExperimentName = 'Rhino_ObsMemXX_';
% Nexp=0;
...
for nVar1 = 1: NumOfSetups1 % Change the explanatory varaiables
    ...
    for nVar2 = 1:NumOfSetups2 % change the 2nd variablesum(nTrials)
        ...
        for nVar3 = 1:NumOfSetups3
            AllParams.BatSonarParams.ClutterGainDB = ClutterGainDB(nVar3);
            ntot = ntot +1 ;

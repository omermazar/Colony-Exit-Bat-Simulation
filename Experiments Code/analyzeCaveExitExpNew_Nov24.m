function [stats, TableCaveExit] = analyzeCaveExitExpNew_Nov24(TableCaveExit, groupBy, lineBy, subplotBy, checkParams, saveResFlg, savePath)
...
%                          xticks(0:1:max(xlim))
                ...
                padding = 0.1;  % Add 10% padding on each side

                % Add padding by adjusting the axis limits
                ax{kSplit}.XLim = ax{kSplit}.XLim + [-1 1] * padding * diff(ax{kSplit}.XLim);
                ax{kSplit}.YLim = ax{kSplit}.YLim + [-1 1] * padding * diff(ax{kSplit}.YLim);
            ...
            if saveResFlg
                % final for save
                fileName = ['CaveExit_New_', currParam];
                
                savefig(fig, fullfile(savePath, fileName))

                set(fig, 'Units', 'centimeters');
                fig.Position([3,4]) = [3.8, 3.8]; % [1.5, 1.9]/1.1;% for small panels 
                sgtitle('')
                ylabel('')
                legend('')
                xlabel('')
...
if saveResFlg
    save(fullfile(savePath, 'StatsNew.mat'), 'stats')
    writetable(TableCaveExit, fullfile(savePath, 'TableCaveExit_New.txt'))
end

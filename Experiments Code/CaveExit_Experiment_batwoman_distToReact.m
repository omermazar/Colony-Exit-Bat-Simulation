function [Table_FullExperiment ] = CaveExit_Experiment_batwoman_distToReact(exeComputer)
...
switch  exeComputer
    case 'batwoman'
        ...
        SavePath   = '/home/omermazar/caveExit_Results'; % '/home/omermazar/caveExit_Results_OJT';% '/home/omermazar/caveExit_Results'; 
        SaveExpDir = '/'; % 'Sep2022\Run\17Oct\';

        % limit the number of parallel CPUs
        numWorkers = 24; % 24 for batWoamn
    ...
    case 'local'
        ...
        % parameter for Saving DATA
        SavePath   = 'C:\Users\YossiYNB3\Documents\University\Research\experiments\caveExit\Results\forFinal\DistToReact\Pre-run'; %'D:\Dropbox\University\????????\experiments\caveExit\Results\';
        SaveExpDir = '/'; % 'Sep2022\Run\17Oct\';
...
%%% React To bats Params
AllParams.BatFlightParams.ReactToBatsFlag = true;
AllParams.BatFlightParams.DistToReactConsp = 0.4; % meters

%%% Var1 - Number Of Bats 
% TotalBatsNumber =  [1 2 5 10 15 20 25 40 100]
TotalBatsNumber =  [2 5 10 20 40]; % [1 2 5 10 20 40 100]; % [1 2 5 10 ]; % [1 2 5 10 20 40 100]; % [1 2 5 10]; % [1 2 5 10 20 40 100]; 
NumOfSetups1    = numel(TotalBatsNumber); % ([Preg, Post])NumOfSetups1 = 2; % ([Preg, Post])
% % num of repetitions in each setup
nTrials         =  [ 120 48 24 12 6]; % [240 120 48 24 12 6 6]; % [4 4 4 4]; % [ 12 6 6]; [240 120 48 24 12 6 6];   
...
% Change the wing-lengthof the bat (0.15 is the nominal TS = -22.5dB)
DistToReactConsp = [0.2 0.4 0.8 1.6 0]; % th last is without any reaction
ReactToBatsFlag  = [true true true true false ];
NumOfSetups2   = length(DistToReactConsp);
var2 = getname(DistToReactConsp);
val2 = eval(var2);
...
% The experiment
ExperimentName = 'CaveExit_PK_ReactDist_'; % 'CaveExit_PK_TwoEars_withFA_';
% Nexp=0;
...
for nVar1 = 1: NumOfSetups1 % Change the explanatory varaiables /
    ...
    for nVar2 = 1:NumOfSetups2 % change the 2nd variablesum(nTrials)
        %         ntot = ntot +1 ;
       AllParams.BatFlightParams.DistToReactConsp = val2(nVar2);
       AllParams.BatFlightParams.ReactToBatsFlag  = ReactToBatsFlag(nVar2);
       v2 = val2(nVar2);
...
%             AllParams.BatFlightParams.MaxVelocity     = manVel(nVar3);
            ...
            %%% Set-UP Paramters
            TS = 10*log10( (AllParams.BatSonarParams.BatWingLength.^2*pi / (4*pi) ));
            SetupParameters = struct( ... 
                'ExperimentName' ,           strrep([ExperimentName, var1, num2str(v1,2), ... 
                                                '_', var2, num2str(v2),  ...
                                                '_', num2str(ntot) ], '.', ''), ...
                ...
                'MothReact2BatsFlag',              AllParams.PreyFlightParams.React2BatsFlag, ...
                ...
                'ConspTargetStrength',          TS,  ...
                'ReactToBatsFlag',             AllParams.BatFlightParams.ReactToBatsFlag, ...
                'DistToReactConsp',            AllParams.BatFlightParams.DistToReactConsp, ...
            ...
            %     'RoomType',                    AllParams.TerrainParams.Type , ...
            %     'DetectConsps',                AllParams.SimParams.DetectConsps, ...
            %     'DetectObs',                   AllParams.SimParamsTS.DetectObs, ...
            %     'DetectPrey',                  AllParams.SimParams.DetectPrey, ...
            %     'MaskingByClutter',            AllParams.SimParams.MaskingByClutter, ...
            %     'MaskingByConsps',             AllParams.SimParams.MaskingByConsps, ...
            %     'MaskingByConspsObsEchoes',    AllParams.SimParams.MaskingByConspsObsEchoes, ...

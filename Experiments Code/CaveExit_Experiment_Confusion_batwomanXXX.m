function [Table_FullExperiment ] = CaveExit_Experiment_Confusion_batwomanXXX()
...
%%%%% NewJune 2024
AllParams.BatSonarParams.ObsClusterFlag = 1;
...
TotalBatsNumber = [2 ]; % [1 2 5 10 20 40 100]; % [1 2 5 10]; % [1 2 5 10 20 40 100]; 
NumOfSetups1    = numel(TotalBatsNumber); % ([Preg, Post])NumOfSetups1 = 2; % ([Preg, Post])
% % num of repetitions i4 4n each setup
nTrials         = [2 ]; % [ 12 6 6]; [240 120 48 24 12 6 6];   
...
% The experiment
ExperimentName = 'CaveExit_PK_ConfusionWithConsObs_New'; % 'CaveExit_PK_TwoEars_withFA_';
% Nexp=0;
...
%                 ' ReceiverType: ',  num2str(AllParams.BatSonarParams.ReceiverType) ]);
            ...
            parfor (nTrialCounter = 1:nTrials(nVar1), numWorkers) % (nTrialCounter = 1:nTrials(nVar1), 12) % Var2NumOfTrials(nVar2) % repeats in each trial

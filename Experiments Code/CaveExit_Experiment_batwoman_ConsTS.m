...
switch  exeComputer
    ...
    case 'local'
        ...
        % parameter for Saving DATA
        SavePath   = 'C:\Users\YossiYNB3\Documents\University\Research\experiments\caveExit\Results\forFinal\PK_ConsTargetStrength\Pre-run'; %'D:\Dropbox\University\????????\experiments\caveExit\Results\';
        SaveExpDir = '/'; % 'Sep2022\Run\17Oct\';
...
TotalBatsNumber = [1 2 5 10 20 40 100]; % [1 2 5 10 20 40 100]; % [1 2 5 10 ]; % [1 2 5 10 20 40 100]; % [1 2 5 10]; % [1 2 5 10 20 40 100]; 
NumOfSetups1    = numel(TotalBatsNumber); % ([Preg, Post])NumOfSetups1 = 2; % ([Preg, Post])
% % num of repetitions in each setup
nTrials         = [240 120 48 24 12 6 6]; % [240 120 48 24 12 6 6]; % [4 4 4 4]; % [ 12 6 6]; [240 120 48 24 12 6 6];   
...
BatWingLength = 0.15 * [1 1/sqrt(10) 1/10 1/sqrt(500)];
NumOfSetups2   = length(BatWingLength);
var2 = getname(BatWingLength);
val2 = eval(var2);
...
% The experiment
ExperimentName = 'CaveExit_PK_TargetStrength_'; % 'CaveExit_PK_TwoEars_withFA_';
% Nexp=0;
...
for nVar1 = 1: NumOfSetups1 % Change the explanatory varaiables /
    ...
    for nVar2 = 1:NumOfSetups2 % change the 2nd variablesum(nTrials)
        %         ntot = ntot +1 ;
       AllParams.BatSonarParams.BatWingLength = val2(nVar2);
       v2 = val2(nVar2);

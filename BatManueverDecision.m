...
%
% function [ManueverType] = BatManueverDecision(FoundPreys , FoundObsicles, PrevManType, PrevManPower)
...
%%%%% This Version is Based on  08Feb2023 - CaveExit  for Paper
...
if isfield(AllParams.BatFlightParams, 'DistToReactConsp')
    MinDist2OtherBat = AllParams.BatFlightParams.DistToReactConsp ; 
else 
...
%% for comppliancae with Confusion Version - New June2024 (Aug2024)
%% New June2024
...
tempManueverPower = "";
currFoundObstacles = FoundObstacles; % the original detections
...
%                     [ManueverCmdStruct.Dist2HuntedPrey, Ind_min] = min(PreyDistances(PreyToHuntIndex));
    ...
    case 'caveExit'          
         ...
         diffSortedDistTH   = 0.5 / xyResolution; % the TH sDistance to check for exit if two objects are angularry closed 
...
%         if strcmp(AllParams.SimParams.TestMode, 'caveExit')
            ...
            if ~exist("edges", 'var')
             ...
             if ~exist("edgeToLandMarkPotential", 'var')
                edgeToLandMarkPotential = [];
            end
            if ~exist("exitPotentialsIx", 'var') || (isempty(exitPotentialsIx) && isempty(edgeToLandMarkPotential))
                exitPotentialsIx = [];
                selectedExitIx = [];
                selectedGap = [];
            end
            if ~exist("sortedXEst", 'var')
                sortedXEst = [];
                sortedYEst = [];
            end
            ...
            if ~exist("selectedGap", 'var')
                selectedGap = [];
            end
            if ~exist("wallOrMark", 'var')
                 wallOrMark = "None";
            end
            
            
            selectedExitIx = selectedGap; 
            if isempty(sortedXEst)
                sortedAngles = [];
                sortedDistancesByAngles = [];
            end

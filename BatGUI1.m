function varargout = BatGUI1(varargin)
% BATGUI1 MATLAB code for BatGUI1.fig
%      BATGUI1, by itself, creates a new BATGUI1 or raises the existing
%      singleton*.
%
%      H = BATGUI1 returns the handle to a new BATGUI1 or the handle to
%      the existing singleton*.
%
%      BATGUI1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BATGUI1.M with the given input arguments.
%
%      BATGUI1('Property','Value',...) creates a new BATGUI1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before BatGUI1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to BatGUI1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help BatGUI1

% Last Modified by GUIDE v2.5 12-Aug-2024 09:55:03
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @BatGUI1_OpeningFcn, ...
                   'gui_OutputFcn',  @BatGUI1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before BatGUI1 is made visible.
function BatGUI1_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to BatGUI1 (see VARARGIN)
...
handles.DataToPlot.DetectObjToAnalyze = 'Prey';
guidata(hObject,handles); % save the changes to the structure 


% Choose default command line output for BatGUI1
...
function varargout = BatGUI1_OutputFcn(hObject, eventdata, handles) 
...
MyBatAnimation(handles.DataToAnalyze, handles.DataToPlot, handles.SaveAminationFlag, handles.Terrain, ...
    handles.AnimationTimeRate, handles.AnimmateCurrentOnly, handles.DataToPlot.DetectObjToAnalyze);
...
FileNameOfDefaultParams = fullfile(path,file);
[handles.SimParams, handles.BatFlightParams, handles.BatSonarParams, ...
    handles.TerrainParams, handles.PreyFlightParams] = InitiateWithDefaultParams(FileNameOfDefaultParams);
% update the GUI wt the new Pararms
[handles] = updateGUIwithLoadedParams(handles, hObject);
...
%
totalD = sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.preyTotalInBeam_1m .* ...
    handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.preyDetectProb_1m, 'omitnan'); 
v13 = sum(totalD )/ sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.preyTotalInBeam_1m,'omitnan');
SummaryString13 = ['Prey Detection Prob 1m Beam: ', num2str(v13,3)];
set(handles.textSummaryGroup13,'String',SummaryString13); %

% Obs
totalI = sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.obsTotalInterference .* ...
      handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.obsTotalInterferenceRatio, 'omitnan'); 
v9 = sum(totalI )/ sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.obsTotalInterference,'omitnan');
SummaryString9 = ['Obs Total Interference Ratio: ', num2str(v9,3)];
set(handles.textSummaryGroup9,'String',SummaryString9); % 
%
totalD = sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.obsTotalInBeam_1m .* ...
    handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.obsDetectProb_1m, 'omitnan'); 
v10 = sum(totalD )/ sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.obsTotalInBeam_1m,'omitnan');
SummaryString10 = ['Obs Detection Prob 1m Beam: ', num2str(v10,3)];
set(handles.textSummaryGroup10,'String',SummaryString10); %

% Consps
totalI = sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.conspsTotalInterference .* ...
    handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.conspsTotalInterferenceRatio, 'omitnan'); 
v11 = sum(totalI )/ sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.conspsTotalInterference,'omitnan');
SummaryString11 = ['Consps Total Interference Ratio: ', num2str(v11,3)];
set(handles.textSummaryGroup11,'String',SummaryString11); %
%
totalD = sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.conspsTotalInBeam_1m .* ...
    handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.conspsDetectProb_1m, 'omitnan'); 
v10 = sum(totalD )/ sum(handles.DataToAnalyze.FlightInterferceSummary.SummaryDataVectors.conspsTotalInBeam_1m,'omitnan');
SummaryString10 = ['Consps Detection Prob 1m Beam: ', num2str(v10,3)];
set(handles.textSummaryGroup12,'String',SummaryString10); %
...
ed1 = get(hObject,'String');
handles.BatFlightParams.DistanceFromPreyToApproach = str2double(ed1);
guidata(hObject,handles) % save the changes to the structure 
...
My_AcousticSig_VecTest (handles.DataToAnalyze, BatNum, axHandle, handles.DataToPlot.DetectObjToAnalyze);
...
BatNum = handles.DataToPlot.BatNumToPlot;
DetectedObj = handles.DataToPlot.DetectObjToAnalyze;
ReceiverType = handles.BatSonarParams.ReceiverType;
Analayze_Sig(handles.DataToAnalyze, PulseNum, BatNum, ReceiverType, DetectedObj)
...
function edit68_ObsMemSize_Callback(hObject, eventdata, handles)
% hObject    handle to edit68_ObsMemSize (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit68_ObsMemSize as text
%        str2double(get(hObject,'String')) returns contents of edit68_ObsMemSize as a double
handles.BatSonarParams.ObsMemorySize = str2double(get(hObject,'String'));
guidata(hObject,handles); % save the changes to the structure 

% --- Executes during object creation, after setting all properties.
function edit68_ObsMemSize_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit68_ObsMemSize (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
PopupContents = cellstr(get(hObject,'String'));
popupValue= PopupContents{get(hObject,'Value')};
handles.DataToPlot.DetectObjToAnalyze = popupValue;
guidata(hObject,handles);% save the struct
...
handles.SimParams.MaskingByConspsPreyEchoes = get(hObject,'Value');
guidata(hObject,handles) % save the changes to the structure 
...
handles.SimParams.MaskingByConspsObsEchoes = get(hObject,'Value');
guidata(hObject,handles) % save the changes to the structure 


% --- Executes on button press in radiobuttonConspsEchoesPlot.
function radiobuttonConspsEchoesPlot_Callback(hObject, eventdata, handles)
% hObject    handle to radiobuttonConspsEchoesPlot (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

handles.DataToPlot.ConspsEchosFlag =get(hObject,'Value');
guidata(hObject,handles)

% Hint: get(hObject,'Value') returns toggle state of radiobuttonConspsEchoesPlot
...
ed1 = get(hObject,'String');
handles.SimParams.swarm_initial_xbat_max = str2double(ed1);
guidata(hObject,handles); % save the changes to the structure 
...
detDist = str2double(get(handles.editDetectRange, 'String'));

DetectedObj = handles.DataToPlot.DetectObjToAnalyze;
[stats] = myDetectionAnalysis(handles.DataToAnalyze, detDist, DetectedObj);
display(stats);
assignin('base','statsDetection', stats);

guidata(hObject,handles); % save the changes to the structure 


function edit70_detDist_Callback(hObject, eventdata, handles)
% hObject    handle to edit70_detDist (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit70_detDist as text
%        str2double(get(hObject,'String')) returns contents of edit70_detDist as a double


% --- Executes during object creation, after setting all properties.
function edit70_detDist_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit70_detDist (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
ed1 = get(hObject,'String');
handles.BatSonarParams.ClutterPointReflecetingArea = str2double(ed1);
guidata(hObject,handles); % save the changes to the structure 
...
% --- Executes on button press in pushbutton_NextCall.
function pushbutton_NextCall_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_NextCall (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
PulseNum = str2double(get(handles.edit64_pulseToAnalyze, 'String')) +1;
set(handles.edit64_pulseToAnalyze, 'String', num2str(PulseNum));
BatNum = handles.DataToPlot.BatNumToPlot;
axHandle =gca;
% if handles.checkbox41_NewFig.Value == 0
%     axHandle = handles.xyPlot;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.SimParams.MaskingByConspsConspsEchoes = get(hObject,'Value');
guidata(hObject,handles) % save the changes to the structure 
...
% --- Executes on button press in TwoEarsFlag_checkBox.
function TwoEarsFlag_checkBox_Callback(hObject, eventdata, handles)
% hObject    handle to TwoEarsFlag_checkBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of TwoEarsFlag_checkBox
handles.BatSonarParams.TwoEarsReception =  get(hObject,'Value');
guidata(hObject,handles) % save the changes to the structure 

% --- Executes on button press in ClutterSideEst_checkbox.
function ClutterSideEst_checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to ClutterSideEst_checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.BatSonarParams.ClutterSideEstimate  =  get(hObject,'Value');
guidata(hObject,handles) % save the changes to the structure 
% Hint: get(hObject,'Value') returns toggle state of ClutterSideEst_checkbox


% --- Executes on selection change in clutterLocInput_popupmenu.
function clutterLocInput_popupmenu_Callback(hObject, eventdata, handles)
% hObject    handle to clutterLocInput_popupmenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
PopupContents = cellstr(get(hObject,'String'));
popupValue= PopupContents{get(hObject,'Value')};
handles.BatSonarParams.ClutterEstSignalIn = popupValue;
guidata(hObject,handles) % save the changes to the structure 

% Hints: contents = cellstr(get(hObject,'String')) returns clutterLocInput_popupmenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from clutterLocInput_popupmenu


% --- Executes during object creation, after setting all properties.
function clutterLocInput_popupmenu_CreateFcn(hObject, eventdata, handles)
% hObject    handle to clutterLocInput_popupmenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.pushbuttonRunSim.Enable = "on";
guidata(hObject,handles);
...
% default file
path = pwd; % 'C:\Users\YossiYNB3\Documents\Omer\Bat Simulation\DATA'
path = strcat(path, '\DATA');
file = 'DefaultParamsTable.xlsx';
if ~isfile(fullfile(path,file))
    disp('default file not found pleas choose filne for default params:')
    [file, path] = uigetfile('*.xls*');
end % if ~isfile
FileNameOfDefaultParams = fullfile(path,file);
[handles.SimParams, handles.BatFlightParams, handles.BatSonarParams, ...
...
handles.BatSonarParams.ConfusionObs  =  get(hObject,'Value');
guidata(hObject,handles) % save the changes to the structure 
...
handles.BatSonarParams.ObsClusterFlag  =  get(hObject,'Value');
guidata(hObject,handles) % save the changes to the structure 
...
ed1 = get(hObject,'String');
handles.BatSonarParams.minClusterTH  = str2double(ed1);
guidata(hObject,handles); % save the changes to the structure 
...
% --- Executes on button press in pushbutton32_CloseAll.
function pushbutton32_CloseAll_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton32_CloseAll (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
 % Get the handle of the GUI
 gui_handle = handles.figure1; % Adjust 'figure1' to the actual Tag name of your GUI figure

 % Find all figure handles
 all_figures = findall(0, 'Type', 'figure');

 % Close all figures except the GUI itself
 for i = 1:length(all_figures)
     if all_figures(i) ~= gui_handle
         close(all_figures(i));
     end
 end

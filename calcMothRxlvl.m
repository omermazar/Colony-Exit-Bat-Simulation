function RxPreyStruct = ...
    calcMothRxlvl...
    (FindsStruct, ObsOrPreyFlag , CurrentFreqs, nTime, CurrentPulseNum, varargin)
%%
%% NEW for Jamming Moths
% this function calcultes and returns the Rx level the the prey items
% receive the bat's calls
...
% 16/04/22 updated by omer : Adding bi-stat detection output
...
RxInd = find( BATTrantmitter.OtherBatsPolarCoordinates(CurrentPulseNum).TargetsID == BatRxNum );
DistVec = BATTrantmitter.OtherBatsPolarCoordinates(CurrentPulseNum).Distances(RxInd);
AngleVec = BATTrantmitter.OtherBatsPolarCoordinates(CurrentPulseNum).TargetAngle(RxInd);
...
DirectInterfernceTimes = min(Bat1TransmitPulseTimes +DelayTimes, MaxTime) ; % Protecting EndTime
 ...
 %% Recontruct the Acoustic Signal in BATReciever for the Direct Interdfrence Vec
    ...
    if AllParams.SimParams.AcousticsCalcFlag
        ...
        BATReciever.AcousticSig_Interference = Add_currEcho_to_full_vec( ...
            BATReciever.AcousticSig_Interference , Curr_Inter_Acoustic, idx1, nSamples);
    else
        BATReciever.AcousticSig_Interference = [];
...
AcousticSig_Interference = BATReciever.AcousticSig_Interference;

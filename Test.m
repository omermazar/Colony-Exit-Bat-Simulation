function [FullVec] =Test(FlightStageCell, StartPulseTimes, NumOfSamples)

FullVec= repmat({''},1,NumOfSamples);

% FullVec(StartPulseTimes) = FlightStageCell;
for kk = 1:length(StartPulseTimes)-1
    FullVec(StartPulseTimes(kk): StartPulseTimes(kk+1)) = FlightStageCell(kk);
end % for kk
FullVec(StartPulseTimes(end):end) = FlightStageCell(end);
